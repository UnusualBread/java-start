package theory.M02_code_organization.S03_object_oriented_programming.MM03_other_concepts.SS01_enumerations.L02_fields_and_methods_in_enum;

/*
Your task is to implement the method changeBalance to control balances.
    - If the given operation is DEPOSIT the method adds the sum to the
      account's balance.
    - If the operation is WITHDRAW the method subtracts the given sum from the
      account's balance. If the given sum is greater than the balance, the
      method shouldn't change it. Instead, the method must output the string
      "Not enough money to withdraw." (without quotes, don't forget the full
      point).
...
public class T03_MoneyOperationsPerformer {
    ...
    public static boolean changeBalance(Account account, Operation operation, Long sum) {
        boolean isChanged = switch (operation) {
            case DEPOSIT -> {
                account.balance += sum;
                yield true;
            }
